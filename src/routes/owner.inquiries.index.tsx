...
function InquiriesList() {
  ...
  const { language, t, translateInquiryStatus } = useLanguage();

  useEffect(() => {
    document.title = language === "pl"
      ? "Zapytania — Panel właściciela"
      : "Inquiries — Owner panel";
  }, [language]);

  const visible = inquiries.filter((i) => filter === "all" || i.status === filter);

  const statusList: InquiryStatus[] = ["new", "read", "answered", "archived"];
  const dateLocale = language === "pl" ? "pl-PL" : "en-US";

  return (
    <div className="font-sans">
      <h1 className="mb-6 font-display text-3xl">{t("owner.inq.title")}</h1>

      <div className="mb-4 flex flex-wrap gap-2">
        <FilterChip label={t("owner.inq.all")} active={filter === "all"} onClick={() => setFilter("all")} />
        {statusList.map((s) => (
          <FilterChip
            key={s}
            label={translateInquiryStatus(s)}
            active={filter === s}
            onClick={() => setFilter(s)}
          />
        ))}
      ...
      {visible.length === 0 ? (
        <div className="rounded-2xl border border-dashed border-border bg-card p-10 text-center text-muted-foreground font-sans">
          {t("owner.inq.empty")}
        </div>
      ) : (
        <ul className="space-y-3 font-sans">
          {visible.map((i) => {
            const tenantsLabel = language === "pl"
              ? (i.tenants === 1 ? "najemca" : "najemców")
              : (i.tenants === 1 ? "tenant" : "tenants");
              ...
              <li key={i.id}>
                ...
                >
                  <div className="flex flex-wrap items-start justify-between gap-2">
                    ...
                    <span
                      className={`shrink-0 rounded-full px-2.5 py-1 text-xs font-medium ${STATUS_TONE[i.status]}`}
                    >
                      {translateInquiryStatus(i.status)}
                    </span>
                  </div>
                  <div className="mt-3 flex flex-wrap gap-x-4 gap-y-1 text-xs text-muted-foreground">
                    <span>{new Date(i.createdAt).toLocaleString(dateLocale)}</span>
                    <span>
                      {i.moveIn} → {i.moveOut}
                    </span>
                    <span>
                      {i.tenants} {tenantsLabel}
                    </span>
                  </div>
